...
int checkboot(cmd_tbl_t *cmdtp, char *dev)
{
	ulong *ptr = (ulong *)0x80c00000;
	image_header_t *im_hd = (ulong *)0x80c00000;
	u32 magic = 0;
	u32 confirm = 0x56190527;
	// u32 confirm = 0x746d4346;
	u32 kernel_size = 0;
	int ret = -1;
	int argc = 5;
	char *argv[5];
	...
	if (0 == strcmp(dev, "usb"))
	{
		ret == do_usb(cmdtp, 0, argc, argv);
	}
#endif
#ifdef MTK_MSDC
	if (0 == strcmp(dev, "mmc"))
	{
		ret = do_mmcops(cmdtp, 0, argc, argv);
	}
#endif
	if (0 != ret) return ret;
	memcpy(&magic, ptr, sizeof(magic));
	memcpy(&kernel_size, ptr + 3, sizeof(kernel_size));
	// memcpy(&kernel_size, ptr + 1, sizeof(kernel_size));

	// magic not found
	if (magic != confirm)
	{
		printf("Magic not match, magic=0x%x \n", magic);
		return -1;
	}

	if ( 0 == kernel_size)
	{
		printf("Kernel size is zero, return. \n");
		return -1;
	}
	memset(&block_size_s, 0, sizeof(block_size_s));
	sprintf(&block_size_s, "0x%x", ntohl(kernel_size)/512 + 1);
	printf("Kernel %d, %s, loading from mmc ...\n", kernel_size, block_size_s);
	printf("Kernel 0x%x, %d, loading from mmc ...\n", ntohl(im_hd->ih_size), ntohl(im_hd->ih_size)/512 + 1);

	// load linux kernel and try boot .
	argc = 5;
	argv[1] = "read";
	argv[2] = "0x80c00000";
	argv[3] = "0x10";
...
void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	unsigned char BootType='6', confirm=0;
	int my_tmp;
	char addr_str[11];
...
/*config bootdelay via environment parameter: bootdelay */
	...
	if(BootType == '3') {
		...
		case '2':
			...
			argc= 3;
			setenv("autostart", "no");
			printf("before do_tftpb argv1:%s,argv2:%s\n",argv[1],argv[2]);
			do_tftpb(cmdtp, 0, argc, argv);
			printf("after do_tftpb ,argv1:%s,argv2:%s\n",argv[1],argv[2]);
		...
		case '6':
			// printf("\n%d: Customized boot.\n", SEL_CUSTOME_BOOT);
#ifdef AXP209_POWER
			printf("Init axp209 power manager.\n");
			int power_failed = 0;
			...
			{	// read from usb
				printf("Found USB devices, try boot from USB/TF card...\n");
				checkboot(cmdtp, "usb");
			}
			printf("No bootable USB Storage found.\n");
#endif
#ifdef MTK_MSDC
			argc = 3;
			argv[1] = "0x32";
			argv[2] = "value";
			printf("Check TF card slot status.\n");
			if (0 == do_gpio(cmdtp, 0, argc, argv))
			{
				printf("TF card in slot, try boot from TF card...\n");
				checkboot(cmdtp, "mmc");
			}
#endif
			// if boot from mmc failed.
			char *argv[2];
			sprintf(addr_str, "0x%X", CFG_KERN_ADDR);
			argv[1] = &addr_str[0];
